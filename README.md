...
# ESP8266 OSC Haptics
A quick and cheap way to add haptics to your VR headset or body for social VR apps. I've designed this to work with ChilloutVR and along with [kafeijao’s](https://github.com/kafeijao/Kafe_CVR_Mods/tree/master/OSC) OSC mod. It's more or less a "native" approach, utilizing OSC integration in these social VR apps. Future plans include; easier configuration and the ability for it to be used with all apps like VRC, Neos, etc.  
...
**(Optional) Push button:** Use a normally open push button. Connect one pin to GND and the other to the designated pin on the dev-board (default is D1).
...
- `ssid` and `password`, will be the name and password of your wifi AP. (Lines 7-8)
- `OSCip` This will be the IP address of the computer running the VR application. (Line 18)

If you are using this in "**Basic Mode**" (Velocity Calculation Disabled) then set `VelEn` equal to "0" (Line 32). Changing the OSC paths won't be necessary as you can remap these with ETC Labs OSC Router.

**However**, If you want to feel how fast the haptic interaction is then set `VelEn`to "1". This sends out a lot of data that overwhelms the OSC Router, so we will need to use [VRChat Open Sound Control Router](https://github.com/SutekhVRC/VOR) by SutekhVRC, at the moment we can't remap OSC addresses like OSC Router so you will have to plugin the OSC addresses for the avatar (which we will create in **Avatar Setup**). 

- `OSCpathHap0` will be the OSC address for the base toggle. (Line 23)
- `OSCpathHap0Pos` will be the OSC address for the position of the pointer in Hap0. (Line 24)
Here's an example: ![ESP_Firmware_OSC_Paths](https://github.com/NulledFloof/ESP8266-VR-OSC/blob/main/Images/ESP_Firmware_OSC_Paths.png)
...
- **First**, add an entry to your Advanced Avatar settings list and give it a name with no spaces. The `Type` field should be set to "Game Object Toggle" and `Generate Type` should be set to "Generate Bool". `Default` and `Use Animation` boxes should be left unselected for this tutorial. You can leave the "GameObjects'' list empty, however, you can add an object or animation to give other in-game users visual confirmation. 
- Add another entry with the same name as before but add "position" to the end of it. The `Type` field should be set to "Input Vector 2" and `Default` should both be set to "0". 
...
- Next, under "On Stay Trigger" set `Setting Name` to the second AAS entry with "position-x" at the end of its name. `Update Method` should be set to "Set From Position". `Min Value` should be set to "0" and `Max Value` should be set to "1".
...
## Software
For "**Basic Mode**" You will need to download and run ETC Labs [OSC Router](https://github.com/ETCLabs/OSCRouter).
...
For "**Advanced Haptics**" Download and install [VRChat Open Sound Control Router](https://github.com/SutekhVRC/VOR) by SutekhVRC.  
...
- Add two entries for the OSC address, it should be `/avatar/parameters/(Your AAS Haptics Name)`and `/avatar/parameters/(Your AAS Haptics Name position-x,y,or,z)` These names should match up with what you setup in your AAS in Unity.
